...
def detail_page(item):
    ...
    try:
        ...
        import re
        key = item.get('id', '')
        if not re.fullmatch(r'(sbb|iskur)-[a-f0-9]{24}', key):
            return None
    if url.scheme != 'https' or url.hostname not in {'kariyerkapisi.gov.tr','kamuilan.sbb.gov.tr','www.iskur.gov.tr','iskur.gov.tr'} or url.username or url.password:
        return None
